...
// ----------- constructor -----------
Network::Network(const char *ssid, const char *password, Gripper &g)
    : wifi_ssid(ssid),
...
// static GripperState g_gripper_state = GripperState::CLOSED;
...
err_t Network::sendResponse(tcp_pcb *pcb, const char *text)
{
  if (!pcb)
    return ERR_CONN;
...
// tilføj efter behov
err_t Network::handleCommand(tcp_pcb *pcb, const char *cmd)
{
  if (std::strcmp(cmd, "OPEN") == 0) // strcmp sammenligner om command og "OPEN" er ens
  {
    if (!gripper.open())
      return sendResponse(pcb, "ERROR\n");
    // apply_gripper_state(GripperState::OPEN);
    return sendResponse(pcb, "OK OPEN\n");
  }
  ...
  if (std::strcmp(cmd, "CLOSE") == 0)
  {
    ...
    return sendResponse(pcb, "OK CLOSED\n");
  }
  ...
  //   return sendResponse(pcb, response);
  // }
...
void Network::trimCommand(char *buf)
{
...
err_t Network::onClientConnected(void *arg, tcp_pcb *client, err_t err)
{
  if (!instance)
    return ERR_OK;
...
err_t Network::onTcpReceive(void *arg, tcp_pcb *pcb, pbuf *packet, err_t err)
{
  if (!instance)
    return ERR_OK;
...
void Network::onTcpError(void *arg, err_t err)
{
  if (instance)
